...
variant = [r'?variant=lang-de_fulltext'   r'' r'?variant=lang-en_fulltext'  ]
outdir = r'./out/'
...
for d in documents:
    for e in variant:
        r = requests.get(token_base_url + str(d['id']) + e)
        if r.ok:
        ...
        if 'application/octet-stream' in rpdf.headers.get('content-type'):
            ...
            if rpdf.content[0:5] == b"%PDF-":
                filename = outdir + d['doc_num'] + '_' + str(d['id']) + '_' +  d['title@de_DE_formal'] + '.pdf'
