...
#!nix-shell -i python3 -p python3Packages.requests -p python3Packages.unidecode
# -*- coding: utf-8 -*-
...
debugBridgePassword="passwordGoesHere"
...
def main():
    with zotero() as z:
        ...
        for d in documents:
            ...
            title = f"{d['doc_num']} - {d['title']['rendered']}"
            tags = d['Untergruppe@de_DE_formal'].split(',')
            url = d['link']

            print(title, tags, url)

            article = journalArticle(
                    publicationTitle = d["Gruppe@de_DE_formal"],
                    volume=d["Ausgabe@de_DE_formal"],
                    issue=d['doc_num'],
                    title=title,
                    url=url)
